...
class SelectEventTypeStep implements StepInterface
{
    ...
    public const STEP_KEY = 'selectEventType';
    ...
    {
        if (!in_array($message, config('steps.events'), true)) {
    ...
    private function getAnswer(string $messenger, string $lang, ?string $error, bool $isEdit, BotContext $context): string
    {
        ...
        $textKey = $isEdit
            ? 'task_change_event_cath'
            : 'task_choose_event_cath';
        ...
        $answer = $context->messenger->format(
            $this->textResolver->header($messenger, $lang, 'eventing', 'task_step_type'),

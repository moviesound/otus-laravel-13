...
final class SelectQuarterlyPeriodStartMonthStep implements StepInterface
{
    public const STEP_KEY = 'selectQuarterlyPeriodStartMonth';
        ...
        if (str_starts_with($message, 'miq_')) {
            ...
            if (!in_array($message, [1, 2, 3])) {
                ...
                return StepResultFactory::switch(
                    SetQuarterlyDeadlineDayStep::STEP_KEY,
                    [
                        'start_month_in_quarter' => $message,
                        'repeat_type' => 'quarterly',
                        'year_type' => 'period',
                    ]
                );
    ...
    public function show(BotContext $context, ?string $error = null): void
    {
        ...
        $body = $this->textResolver->get(
            $type === 'task'
                ? 'choose_start_month_in_quarter'
                : 'choose_start_month_in_quarter_event',
            $messenger,
            $lang
        );
